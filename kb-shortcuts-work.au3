...
Func out_task_ticket()
  ...
  Local $repliconAccount = "Be 100% sure you know the Replicon account to use for this task. To start, look in the ADO ticket's 'Time Category' section, then be sure you know exactly which Replicon account it corresponds to. If you are not 100% sure, ask."
  Local $oneClick = "If/When I confirm the ticket will require code edits, add 1-click sub-task reminders to ADO ticket, then set Coding sub-task to myself/Active."
	...
	Local $updateTicket = "Update ticket{ENTER}{TAB}Update ticket with resolution. This will be included in 'release notes,' which are read by the deployment team and end users. Add to ADO's standalone 'Resolution' section. " & $regSetting & "{ENTER}Update ticket with verification steps. Add this to 'Discussion' section.  Be sure to add screenshots where helpful.{ENTER}Update ticket with release comment (in ADO, add this to the standalone 'Resolution' section).{ENTER}" & $sSql & "{ENTER}Ensure ticket resolution section describes the edit required.{ENTER}Ensure ticket verification section describes to QA the edit required, as well as where the script is located, and exactly what tables/columns are updated.  If not, QA will have problems knowing exactly what statement to execute.{ENTER}{ENTER}{ENTER}"
  ...
  SendEx($sNetworkreqs & "{ENTER}When I begin working on the ticket, open the scrum board and move the ticket to the In Dev column.{ENTER}" & $sourceBranch & "{ENTER}If helpful, try to use git bisect to determine the breaking commit.{ENTER}")
  SendEx($oneClick & "{ENTER}{ENTER}")
  SendEx("T: If ticket needs another team (e.g. SWAT) to obtain more info from customer, or ask customer to check some things on their end, move status to ‘In Review’ in the scrum board.{ENTER}")
    Sleep(200)
