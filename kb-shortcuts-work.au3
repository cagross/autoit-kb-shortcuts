...
Func out_task_ticket()
	...
	Local $updateTicket = "Update ticket{ENTER}{TAB}Update ticket with resolution. Add to ADO's standalone 'Resolution' section. " & $regSetting & "{ENTER}Update ticket with verification steps. Add this to 'Discussion' section.  Be sure to add screenshots where helpful.{ENTER}Update ticket with release comment (in ADO, add this to the CR Release Notes section).{ENTER}" & $sSql & "{ENTER}Ensure ticket resolution section describes the edit required.{ENTER}Ensure ticket verification section describes to QA the edit required, as well as where the script is located, and exactly what tables/columns are updated.  If not, QA will have problems knowing exactly what statement to execute.{ENTER}{ENTER}{ENTER}"
