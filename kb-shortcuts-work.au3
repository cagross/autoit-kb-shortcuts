...
Func send_crv()
	...
	Send("^b^uResolution^u^b{ENTER}{ENTER}{ENTER}")
...
Func out_task_ticket()
	...
	Local $regSetting = "If I add, modify, or delete a reg setting, add that info to CR Release Notes section."
	...
	SendEx("Make edits{ENTER}{TAB}Check if ticket will require module conversion.  If so, and time permits, start with that.{ENTER}Try to use unit tests (maybe with ChatGPT){ENTER}Add JSDoc{ENTER}" & $sSecuritydalfunc & "Wait for sprint to officially start.{ENTER}Create local feature branch (syntax: feature/<number>) and commit edits there. If someone else made the edits (e.g. J), note that in the Git commit message.{ENTER}" & $sAppserveredits & "{Enter}{ENTER}Wait for commits to be made in local feature branch.{ENTER}Rebase feature branch onto target branch if helpful/necessary.{ENTER}Create new PR by pushing feature to remote{ENTER}Wait for code review to complete.{ENTER}Update ticket{ENTER}{TAB}Update ticket with resolution.{ENTER}Update ticket with verification steps. Be sure to add screenshots where helpful.{ENTER}Update ticket with release comment (in ADO, add this to the CR Release Notes section).{ENTER}" & $regSetting & "{ENTER}If task required modification of database (either editing schema, or data), be sure to take correct action. That action is:{ENTER}{TAB}Compose the SQL statement you think will accomplish what you want.{ENTER}Send it to Jaheer for his approval. Do not continue without his approval.{ENTER}Find the SharePoint folder for Internal Releases, then find the folder for the release in-question. In that folder, should be folders for DB Schema Scripts and DB SQL Scripts.  The former is for schema edits and the latter is for data edits.  Open the desired folder.{ENTER}In that folder should be Word docs corresponding to each of the three databases. Open the Word doc corresponding to the DB in-question and paste the SQL statement.  Save and close.{ENTER}Ensure ticket resolution section describes the edit required.{ENTER}Ensure ticket verification section describes to QA the edit required, as well as where the script is located, and exactly what tables/columns are updated.  If not, QA will have problems knowing exactly what statement to execute.{ENTER}{ENTER}{ENTER}Assign to QA.{ENTER}Wait for QA to pass.{ENTER}Manually complete PR/merge.{ENTER}{ENTER}")
